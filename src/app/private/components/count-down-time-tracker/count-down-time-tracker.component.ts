...
import {
  errorImageSelect,
  successImageSelect,
} from 'src/app/shared/types/image-switch';
...
export class CountDownTimeTrackerComponent implements OnInit, OnDestroy {
  ...
  calculateRemainingTime() {
    ...
    const differenceInMilliseconds = Math.abs(
      now.getTime() - this.ticketValidationDate.getTime()
    );
    const maxTimeInMs = 15 * 60 * 1000; // 15 minutes in milliseconds

    this.diffDate = new Date();
    this.diffDate.setMinutes(
      Math.max(0, maxTimeInMs - differenceInMilliseconds) / (1000 * 60)
    );
    this.diffDate.setSeconds(
      (Math.max(0, maxTimeInMs - differenceInMilliseconds) % (1000 * 60)) / 1000
    );
  }
